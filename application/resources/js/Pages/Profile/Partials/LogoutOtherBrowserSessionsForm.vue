    ...
    <ActionSection>
        ...
        <template #content>
            ...
            <div class="flex items-center mt-5">
                ...
                <v-chip class="ms-3" size="x-large" color="success"
                        v-if="form.recentlySuccessful">Done
                </v-chip>
            </div>

            <v-dialog v-model="confirmingLogout" width="512">
                <v-card>
                    ...
                    <v-card-text>
                        <v-text-field type="password" v-model="form.password" placeholder="Password" autocomplete="current-password" @keyup.enter="logoutOtherBrowserSessions" />
                        <v-alert type="error" class="mt-2" v-if="form.errors.password">{{form.errors.password}}</v-alert>
                    </v-card-text>
                    <v-card-actions>
                        <v-btn color="secondary" @click="closeModal">Cancel</v-btn>
                        <v-btn color="primary" @click="logoutOtherBrowserSessions" :loading="form.processing">Log Out Other Browser Sessions</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
        </template>
